...
export function FranchiseLayout({ children }: FranchiseLayoutProps) {
  ...
  const { signOut, franchiseInfo } = useAuth();
          ...
          <div className={cn("flex items-center gap-3", !sidebarOpen && "lg:justify-center")}>
            ...
            {sidebarOpen && (
              <div className="overflow-hidden">
                <h1 className="font-bold text-sidebar-foreground text-lg truncate">
                  {franchiseInfo?.name || 'Franchise'}
                </h1>
                <p className="text-xs text-muted-foreground">Rekap Penjualan</p>
              </div>
            )}
          </div>
          <button
